...
from shop.serializers.createscoop import ScoopSerializer
...
class ScoopViewSet(GenericViewSet):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            for p in plugin_pool.get_all_plugins():
                if issubclass(p, ScoopFormPlugin):
    ...
    def upload(self, request):
        ...
        product = Product.objects.get_from_request(request)
        dialog_data = []
        ...
        with transaction.atomic():
            for form_class, data in dialog_data:
                form = form_class.form_factory(request, data, Product)
                if form.is_valid():
