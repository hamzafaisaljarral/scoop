...
from .models import Hire,Subscription



class Hireadmin(admin.ModelAdmin):
    list_display = ('hiredprofile','jobhiredfor','profileofjobpublisher','message','price','is_accepted','is_rejected','end_date', )

    fieldsets = (
        (None, {
            'fields': (
                'hiredprofile', 'jobhiredfor', 'profileofjobpublisher', 'message','price','is_accepted','is_rejected','end_date',
