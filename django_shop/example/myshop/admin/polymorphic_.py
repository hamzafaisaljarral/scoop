...
class ScoopAdmin(InvalidateProductCacheMixin, SortableAdminMixin, FrontendEditableAdminMixin,
                     CMSPageAsCategoryMixin, PlaceholderAdminMixin, PolymorphicChildModelAdmin):
    base_model = Product
    fields = ['product_name', 'slug', 'product_code', 'unit_price', 'active', 'caption',
              'description', 'manufacturer', 'time_duration']
    filter_horizontal = ['cms_pages']
    inlines = (ProductImageInline,)
    prepopulated_fields = {'slug': ['product_name']}
...
class ProductAdmin(PolymorphicSortableAdminMixin, PolymorphicParentModelAdmin):
    base_model = Product
    child_models = [SmartPhoneModel, SmartCard, Commodity, Scoop]
